...
func (_q *APIKeyQuery) QueryConversations() *ConversationQuery {
...
func (_q *APIKeyQuery) WithConversations(opts ...func(*ConversationQuery)) *APIKeyQuery {
	query := (&ConversationClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *APIKeyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*APIKey, error) {
	...
	if query := _q.withConversations; query != nil {
		if err := _q.loadConversations(ctx, query, nodes,
			func(n *APIKey) { n.Edges.Conversations = []*Conversation{} },
			func(n *APIKey, e *Conversation) { n.Edges.Conversations = append(n.Edges.Conversations, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *APIKeyQuery) loadConversations(ctx context.Context, query *ConversationQuery, nodes []*APIKey, init func(*APIKey), assign func(*APIKey, *Conversation)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*APIKey)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(conversation.FieldAPIKeyID)
	}
	query.Where(predicate.Conversation(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(apikey.ConversationsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
