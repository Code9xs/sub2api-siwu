...
func (_q *ConversationQuery) QueryAPIKey() *APIKeyQuery {
...
func (_q *ConversationQuery) WithAPIKey(opts ...func(*APIKeyQuery)) *ConversationQuery {
	query := (&APIKeyClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ConversationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Conversation, error) {
	...
	if query := _q.withAPIKey; query != nil {
		if err := _q.loadAPIKey(ctx, query, nodes, nil,
			func(n *Conversation, e *APIKey) { n.Edges.APIKey = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *ConversationQuery) loadAPIKey(ctx context.Context, query *APIKeyQuery, nodes []*Conversation, init func(*Conversation), assign func(*Conversation, *APIKey)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*Conversation)
	for i := range nodes {
		fk := nodes[i].APIKeyID
...
func (_q *ConversationQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withAPIKey != nil {
			_spec.Node.AddColumnOnce(conversation.FieldAPIKeyID)
		}
	}
